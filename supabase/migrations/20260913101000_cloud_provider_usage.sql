...
create function public.cloud_provider_usage(p_task uuid,p_owner uuid,p_generation bigint,p_key text,p_usage jsonb) returns void
language plpgsql security invoker set search_path=public as $$
declare rid uuid;
begin
 perform cloud_assert_lease(p_task,p_owner,p_generation);
 if p_key !~ '^usage:[0-9a-f-]{36}:(enrichment|analysis)$' then raise exception 'Invalid usage key';end if;
