create table public.analysis_results (
 id uuid primary key default gen_random_uuid(), pipeline_job_id uuid not null unique references public.pipeline_jobs(id),
 source_record_id uuid not null references public.source_records(id), catalog_version_id text not null references public.catalog_versions(id),
 rules_version text not null, classification text not null check(classification in ('in_portfolio','out_of_portfolio','discarded','insufficient_evidence')),
 reasons jsonb not null, explanation text not null, service_description text not null, target_population text not null,
 evidence jsonb not null, reviewed_classification text check(reviewed_classification in ('in_portfolio','out_of_portfolio','discarded','insufficient_evidence')),
...
create function public.persist_analysis(p_job uuid,p_version text,p_result jsonb,p_candidates jsonb,p_evidence jsonb) returns void language plpgsql security invoker set search_path=public as $$
...
begin
 ...
 values(p_job,j.source_record_id,p_version,'leaf-normative-v1',p_result->>'classification',p_result->'reasons',p_result->>'explanation',p_result->>'service_description',p_result->>'target_population',p_evidence);
