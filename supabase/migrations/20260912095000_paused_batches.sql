...
begin
 ...
 update pipeline_jobs j set status='ready',error_message=null where run_id=p_run and status='error' and preparation_status='ready' and not exists(select 1 from analysis_results a where a.pipeline_job_id=j.id);
