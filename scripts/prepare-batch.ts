...
async function main() {
  try {
    ...
    await supabase.from("pipeline_jobs").update({ status: "preparing", preparation_status: "discovering", preparation_message: null }).eq("run_id", runId).match(process.env.WORKFLOW_JOB_ID?{id:process.env.WORKFLOW_JOB_ID}:{}).in("status", ["selected", "error"]);
    await run("sources:discover", ["--run-id", runId!]);
    await supabase.from("pipeline_jobs").update({ preparation_status: "fetching" }).eq("run_id", runId).match(process.env.WORKFLOW_JOB_ID?{id:process.env.WORKFLOW_JOB_ID}:{}).eq("status", "preparing");
    await run("sources:sample", ["--run-id", runId!, "--limit", "100", ...(useOcr ? ["--ocr"] : [])]);
    await supabase.from("pipeline_jobs").update({ preparation_status: "chunking" }).eq("run_id", runId).match(process.env.WORKFLOW_JOB_ID?{id:process.env.WORKFLOW_JOB_ID}:{}).eq("status", "preparing");
    await run("sources:chunk", ["--run-id", runId!, "--limit", "200"]);

    const { data: jobs, error } = await supabase.from("pipeline_jobs").select("id,source_record_id,status,analysis_results(id),source_documents:source_records(source_documents(status,chunk_count,error_message))").eq("run_id", runId).match(process.env.WORKFLOW_JOB_ID?{id:process.env.WORKFLOW_JOB_ID}:{});
    ...
    const { data: jobs } = await supabase.from("pipeline_jobs").select("source_record_id").eq("run_id", runId).match(process.env.WORKFLOW_JOB_ID?{id:process.env.WORKFLOW_JOB_ID}:{});
...
async function estimateTokens(id: string) {
  const { data: jobs } = await supabase.from("pipeline_jobs").select("source_record_id").eq("run_id", id).match(process.env.WORKFLOW_JOB_ID?{id:process.env.WORKFLOW_JOB_ID}:{}).eq("status", "ready");
  const recordIds = (jobs ?? []).map((job) => job.source_record_id);
