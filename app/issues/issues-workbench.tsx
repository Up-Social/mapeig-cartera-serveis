...
function IssueRow({ issue }: { issue: IssueRecord }) {
  const { record } = issue;
  return <AccordionItem value={record.id} className="px-4"><AccordionTrigger className="gap-4 py-4 hover:no-underline"><div className="min-w-0 flex-1 text-left"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className="border-neutral-300 bg-neutral-50 text-neutral-950">{ISSUE_CATEGORY_LABELS[issue.category]}</Badge><span className="text-xs text-muted-foreground">{FINANCING_TYPE_LABELS[record.financingType]} · {SOURCE_LABELS[record.sourceDataset] ?? record.sourceDataset}</span>{record.batchNumber && <span className="text-xs text-muted-foreground">Lot {record.batchNumber}</span>}</div><p className="mt-2 font-medium leading-5">{record.title}</p><p className="mt-1 text-xs text-muted-foreground">{displaySourceIdentifier(record.sourceRecordId)}{record.providerName ? ` · ${record.providerName}` : ""}</p><p className="mt-2 line-clamp-2 text-sm text-neutral-900">{issue.message}</p></div></AccordionTrigger><AccordionContent keepMounted className="border-t pb-5 pt-4"><IssueDetail issue={issue} /></AccordionContent></AccordionItem>;
}
...
function IssueDetail({ issue }: { issue: IssueRecord }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
    <div className="min-w-0 space-y-5">
      <section className="rounded-xl border border-neutral-300 bg-neutral-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-neutral-900">Motiu de la incidència</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-neutral-950">{issue.message}</p><dl className="mt-3 grid gap-2 text-xs sm:grid-cols-2"><div><dt className="font-semibold text-neutral-900">Fase</dt><dd className="mt-1">{phaseLabel(issue.phase)}</dd></div><div><dt className="font-semibold text-neutral-900">Data</dt><dd className="mt-1">{formatDate(issue.occurredAt)}</dd></div></dl></section>
