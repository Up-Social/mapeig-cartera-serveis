...
function BatchDetail({ batch }: { batch: BatchSummary }) {
  return <div className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Progrés del lot</p><h3 className="mt-1 text-xl font-semibold">{outcomeLabel(batch)}</h3></div>{batch.reviewCount > 0 && <Link href={`/review?batch=${batch.id}`} className={buttonVariants()}>Obrir revisió</Link>}</div>
    {batch.status === "paused" && <ResumeBatch batch={batch}/>}
...
function ResumeBatch({batch}:{batch:BatchSummary}) {
 ...
 return <div className="rounded-lg border p-4"><p>{batch.pauseReason??'Lot pausat'}</p><Button className="mt-3" disabled={pending} onClick={()=>startTransition(async()=>{try{await runBatchOperation({operation:'resume',batchId:batch.id});setMessage('Lot en cua. El worker continuarà les fases pendents.');}catch(error){setMessage(error instanceof Error?error.message:String(error));}})}>Reprendre lot</Button>{message&&<p className="mt-2 text-sm">{message}</p>}</div>;
}
