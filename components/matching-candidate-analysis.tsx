...
export function MatchingCandidateAnalysis({ candidate }: { candidate: MatchingCandidate }) {
  return <article className={`rounded-xl border p-4 ${candidate.rank === 1 ? "border-neutral-900 bg-neutral-50" : "border-neutral-200"}`}>
    ...
    <section className="mt-4"><h5 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Per què encaixa amb aquest servei</h5><p className="mt-2 whitespace-pre-line text-sm leading-6 text-neutral-700">{candidate.rationale || "Aquest matching no conserva una justificació textual."}</p></section>
