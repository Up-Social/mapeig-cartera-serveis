...
export function MatchingCandidateAnalysis({ candidate }: { candidate: MatchingCandidate }) {
  return <article className={`rounded-xl border p-4 ${candidate.rank === 1 ? "border-neutral-900 bg-neutral-50" : "border-neutral-200"}`}>
    ...
    <details className="mt-3 border-t pt-3"><summary className="cursor-pointer text-xs font-semibold text-neutral-600">Evidència utilitzada ({candidate.evidence.length})</summary>{candidate.evidence.length ? <div className="mt-2 space-y-3">{candidate.evidence.map((item) => <blockquote key={`${candidate.id}-${item.ordinal}`} className="whitespace-pre-wrap break-words border-l-2 border-neutral-300 pl-3 text-xs leading-5 text-neutral-600">{item.content}</blockquote>)}</div> : <p className="mt-2 text-xs text-muted-foreground">Aquest matching no té fragments vinculats.</p>}</details>
