...
export function executionStatus(task:{execution_state:string;lease_until:string|null;last_progress_at:string|null;failure_kind:string|null},now=Date.now()):ExecutionStatus{
 ...
 return {state,label:labels[state]??'Pendent d’inici',lastProgress:task.last_progress_at,recoverable:['pending','paused','interrupted'].includes(state)&&task.failure_kind!=='provider_unknown',reason:task.failure_kind};
}
