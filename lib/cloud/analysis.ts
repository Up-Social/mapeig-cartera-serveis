...
export async function analyzeRecord(c:Context,job:{id:string;source_record_id:string},phase:'enrichment'|'matching'){
 ...
 if(phase==='enrichment'){
  ...
  await commit(c,job.id,'enrichment',{enrichment:result,model,evidence:[...new Set(result.evidence_ordinals)].map(n=>chunks[n-1])});
 }else{
  const catalog=await loadOfficialCatalog(c.db);
  const raw=await providerRequest(c,`ai:${job.id}:matching`,{model,instructions:MATCHING_INSTRUCTIONS,input:buildNormativeInput({...r.data,verified_enrichment:r.data.record_enrichments?.[0]},catalog.eligible,catalog.all,catalog.version.general_context,chunks),text:{format:{type:'json_schema',name:'analysis',strict:true,schema:candidatesOnlySchema()}},max_output_tokens:4000});
  ...
  await commit(c,job.id,'analysis',{version:catalog.version.id,result,candidates:result.candidates.map(candidate=>({...candidate,model,metadata:{response_id:raw.id,usage:raw.usage},evidence:candidate.evidence_ordinals.map(n=>chunks[n-1])})),evidence:result.evidence_ordinals.map(n=>chunks[n-1])});
 }
