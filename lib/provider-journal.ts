...
export type ProviderJournal={claim:()=>Promise<Claim>;save:(id:string,state:'received'|'retry'|'rejected',response:Record<string,unknown>)=>Promise<void>};
export async function executeJournaledCall(journal:ProviderJournal,send:()=>Promise<{status:number;body:Record<string,unknown>}>){
 ...
 if(response.status>=500)throw Error('PROVIDER_UNKNOWN');
...
export async function journaledProviderRequest(db:SupabaseClient,jobId:string,phase:string,body:unknown){
 ...
 return executeJournaledCall({
  ...
  async save(id,state,response){const r=await db.from('provider_calls').update({state,response,received_at:new Date().toISOString()}).eq('id',id).eq('state','sending');if(r.error)throw Error('PROVIDER_UNKNOWN');},
