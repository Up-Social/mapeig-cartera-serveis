...
export async function getComparisonReport(id:string){
 ...
 const records=await Promise.all(members.map(async member=>{
  ...
  return {recordId:member.source_record_id,jobId:job.data.id,originJobId:member.origin_job_id,...compareSnapshots(member.baseline,{...snapshot.data?.payload,phases:phases.data,review:review.data,documents})};
 }));
