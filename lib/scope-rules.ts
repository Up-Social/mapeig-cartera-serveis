...
export function applyScopeRules(original:AnalysisOutput,roles:Partial<RoleFacts>|undefined,chunks:{content:string}[]){
 const literal=(quote:string,ordinals:number[])=>{const exact=quote.trim();const peripheral=exact.replace(/^(?:\.{3}|…)+\s*/,'').replace(/\s*(?:\.{3}|…)+$/,'').trim();return peripheral.length>=12&&ordinals.some(n=>chunks[n-1]?.content.includes(exact)||chunks[n-1]?.content.includes(peripheral));};
 const known=(name:RoleName)=>{const f=roles?.[name];return !!f&&f.state==='known'&&f.kind!=='unknown'&&!!f.value?.trim()&&Array.isArray(f.evidence_ordinals)&&f.evidence_ordinals.length>0&&f.evidence_ordinals.every(n=>Number.isInteger(n)&&n>0&&n<=chunks.length)&&Array.isArray(f.quotes)&&f.quotes.length>0&&f.quotes.every(q=>q.trim().length>0&&literal(q,f.evidence_ordinals));};
